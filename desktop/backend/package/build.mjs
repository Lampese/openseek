...
class Build {
  ...
  async proton(args, env = {}) {
    const manifest = await readFile(join(this.desktop, "backend/moon.mod"), "utf8");
  ...
  async native(profile) {
    ...
    if (this.command === "macos") {
      await this.commandRun("moon", ["build", "backend", "--target", "native", "--target-dir", "target/moonbuild/macos-12.0", ...release], { env });
    ...
    await this.commandRun("moon", ["build", "backend", "--target", "native", ...warning, ...release]);
  ...
  async package(options) {
    ...
    if (this.command === "macos") {
      ...
      env.PROTON_MACOS_ENTITLEMENTS = join(this.desktop, "backend/package/macos",
