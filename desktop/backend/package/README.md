...
`backend/package/build.mjs` acquires and stages the native tools shipped with
...
Development may use a documented system command because `backend/package/dev` does
...
moon -C desktop test backend/internal/host --target native
just check
just test
just build
```
