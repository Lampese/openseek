...
当 `mbtx` 提供 `ptc` 参数时，可用 `ptc: true` 在脚本内计算参数、调用宿主工具并处理结果。内置 `tools` 客户端无需导入：`tools.edit(...)`、`tools.multi_edit(edits)`、`tools.web_search(query)` 或 `tools.call(name, arguments)`。检查 `result.is_error`；工具错误作为值返回，传输失败则抛出异常，不得因此自动重试写操作。搜索在 `result.data` 返回结构化来源；打印选中的结果及 URL，方便下一步引用。只有打印输出进入模型上下文，嵌套调用另存于会话记录。

PTC 保持前台运行（通常最多 300 秒），不能与 `subrun` 或 `escalated` 组合，只开放明确启用的叶子工具。每次脚本最多调用 64 次；有状态调用串行，独立搜索可以并发。简单调用直接使用工具；批量修改保留 `multi_edit` 的验证语义，`edits_file` 仍可使用。当计算或结果筛选能减少模型往返时使用 PTC；完整 API 和限制见工具说明。
