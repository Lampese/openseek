...
宿主仅提供本次运行的连接凭证，不改写源码。统一使用 `@tools.call(name, arguments)`，例如 `@tools.call("edit", arguments)` 或 `@tools.call("web_search", { "query": query })`。参数是与直接调用对应工具完全相同的 JSON 对象，由宿主应用默认值并验证参数。在 `async fn main` 中调用；MoonBit 异步调用直接挂起，不使用 `await` 关键字。进行依赖操作前检查每次返回的 `result.is_error`；工具错误作为值返回。传输失败会抛出异常，但修改可能已经完成：先重新读取受影响的文件，再决定是否重试；不要盲目重跑已经执行过修改的脚本。
