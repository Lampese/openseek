...
当 `mbtx` 提供 `ptc` 参数时，受支持的 wasm 运行默认允许脚本调用宿主工具；设置 `ptc: false` 可关闭。当计算或结果筛选能减少模型往返时使用 PTC：读取数据、计算替换内容、调用 `@tools.multi_edit({ "edits": edits })`，然后验证并打印摘要。简单调用直接使用工具；批量修改保留 `multi_edit` 的验证语义，`edits_file` 仍可使用。多次独立调用不构成一个原子批次。下一步需要判断时，返回模型继续处理。
