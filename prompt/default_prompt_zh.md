...
环境信息在 `References:` 一行给出本次安装的只读 `share/` 资源目录。MoonBit 官方文档（moonbit-docs 的 Markdown 构建版）位于其 `doc/moonbit/` 子目录。关于语言、标准库、工具链和教程的事实，以这些文档为准，先查阅再使用。该目录由当前会话所运行的可执行文件解析得到，不来自配置，因此安装位置移动或升级后依然有效。
...
按需读取工作区外的参考资料：先在该参考目录的 `doc/moonbit/` 下定位相关页面，再按下述方式读取所需片段。
...
| `filename="@builtin/check.mbtx"` | 运行参考目录 `workflow/` 下的内置工作流。 |
