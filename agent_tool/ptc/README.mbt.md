...
Call `mbtx` with `ptc: true` to give a MoonBit script the host's bundled `tools`
client. It invokes the same registered executors as direct tool calls, including
...
No SDK import is required. Existing imports and aliases are preserved. The host
adds imports and helpers to a private staged copy; saved scripts retain their
original source. A source without imports gains one leading line in compiler
diagnostics. Reserve `tools`, `PtcTools`, `PtcResult`, `PtcTransportError`, and
`ptc_*` import aliases for the client.

## Results and search

All calls return `PtcResult { content : String, is_error : Bool, data : Json? }`.
Tool errors are values. Transport failures raise `PtcTransportError`: the tool
...
from script failures. The bundled client and host protocol ship together.
