#!/usr/bin/env python3
"""Bounded live-model PTC prompt A/B. Credentials are inherited, never recorded."""
import argparse
...
def main():
    ...
    for variant in ('baseline', 'candidate'):
        section = (HERE / f'{variant}.md').read_text().rstrip() + '\n\n'
        prompt = base[:start] + section + base[end:]
        (out / f'{variant}.md').write_text(prompt)
        hashes[variant] = hashlib.sha256(prompt.encode()).hexdigest()
    engine = args.engine.resolve()
    manifest = {'model': 'deepseek-v4-flash', 'runs': args.runs, 'max_steps': 24,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as pool:
        futures = [pool.submit(trial, engine, out, *job, args.timeout, args.require_ptc) for job in jobs]
