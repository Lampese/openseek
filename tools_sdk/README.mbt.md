...
[Run the deprecation migration example](examples/README.md). It reads real compiler
...
`moon check --deny-warn` and `moon test`. The same checked-in script runs in the
host integration suite through the published SDK; a second run performs no edits.
